...
**Modular house rework, 2026-09-21 (after Marcel's "I can see every modular piece" report).**
- *Phase 1 (bac166a):* door leaves follow the real doorway (`HouseRecipes.DOOR_OPENING`; `Wall_Door_B` is a door beside a side light, 1.375 off centre; `Wall_Door_C` takes a `Door_Double` leaf), every brick face sits on the grid line (`BRICK_PLANE`, measured from the GLBs, because the export re-centres pieces on their bounding box), and each run of bodies holds a dark core Part (register row EH1). Rough edges 2 (first half), 7 and the see-through corners are closed by it.
- *Phase 2:* the seams were never gaps (0.00 between every pair of modules). Every module maps its brick to the same quadrant of the trim-sheet atlas, so mipmaps bleed the slate beside it into each module edge, wider the further away you stand; and the row back wall and Tier B roof were one quad stretched 6-14 times. `tools/kit/make_house_units.py` (pure Python, reads the recipe lines out of `HouseRecipes.luau`) merges each recipe into `HOUSE__<unit>__masonry / __roof / __trim` (74 meshes: 10 terraced, 4 semis + mirrored halves, 4 detached, 4 gable ends) and crops the brick and slate quadrants into standalone tileable textures (`house_brick_ts1_D.png` ...). Masonry and slate UVs are remapped onto those and multiplied on scaled / stretched pieces, so a wrap lands on brick and bricks are one size on every wall. Generated `Shared/HouseUnits.luau` holds each mesh's bottom-centre offset and the recipe signature; `HouseUnitsTest` fails when a recipe changes without a rebuild. Ids: `asset_ids_house_units.json`, `asset_ids_house_textures.json`; install with `assets/kit/_export/install_house_units.luau` (from `make_house_units.py --install`) into `Kit.Meshes` and `Kit.Appearances`. `Detail.merged` (on in every preset) places them; a lot whose units are not all installed falls back to the module-by-module build, so a fresh place still gets houses. `PART_BUDGET` is now 12,000. Rough edges 2 and 3 are closed by it.
