...
9. Last RunAll in Studio: **404 passed, 0 failed** (09:13). WireDresser, CrossingDresser, GroundInfluence (shared + client) and their tests were created in Studio with multi_edit (comments trimmed there). TownBuilderTest now exists in Studio as a shortened copy (made with multi_edit); the disk file is the full one.
