...
12. May the agent upload new shopfront textures overnight (X101: a takeaway / chippy front for the high street)? Every shopfront is an image uploaded with tools/upload_kit.py, and no new asset was uploaded tonight without asking. *Rec: yes for kit textures made by tools/kit scripts (own art, same pipeline as the existing fronts); list each upload in the log.*
