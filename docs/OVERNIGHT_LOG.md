...
9. Last RunAll in Studio: **419 passed, 0 failed** (11:04). WireDresser, CrossingDresser, CentreTreeDresser, GreeneryDresser, GatewayDresser, ClientKit, Planting, EnumNamesTest, GroundInfluence (shared + client), Swans (client), LitWindows (client), PlayAreaLife (client) and their tests were created in Studio with multi_edit (comments trimmed there). TownBuilderTest now exists in Studio as a shortened copy (made with multi_edit); the disk file is the full one.
