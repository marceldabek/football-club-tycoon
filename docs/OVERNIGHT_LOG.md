...
**Start here (needs you):** 1) click Connect in Studio's Rojo panel, then run RunAll once (disk code was never compiled directly tonight, known bug 4); 2) **save the place** (the phone box, swan and forklift models from 09:24–09:31 live only in it); 3) answer questions 9, 11, 12, 13, 14, 15, 16 and 17 (Weir Road part reserve, how far Low goes, texture uploads, tier dressing); 4) try a mid-range phone on Low and High (X28/X61: town now 21.2k parts; the square shows 19.9k on High, 13.4k on Low); 5) a Players = 2 Studio test for friendlies and trades (I3.1d/I4.1d), and for leaving mid-match / rejoining (X206–X208) and invite/offer cards when a club leaves or a plot changes hands (X209–X211); with five players, the free-ground button for a visitor (X221).
