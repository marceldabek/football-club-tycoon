...
**Start here (needs you):** 1) click Connect in Studio's Rojo panel, then run RunAll once (disk code was never compiled directly tonight, known bug 4); 2) answer questions 9 and 10 (new since 05:58: the Weir Road part reserve and tinted car colours); 3) try a mid-range phone on Low and High (X28/X61: town now 18k+ parts); 4) a Players = 2 Studio test for friendlies and trades (I3.1d/I4.1d).
