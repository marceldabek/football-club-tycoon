...
9. Last RunAll in Studio: **413 passed, 0 failed** (10:10). WireDresser, CrossingDresser, CentreTreeDresser, GreeneryDresser, ClientKit, GroundInfluence (shared + client), Swans (client) and their tests were created in Studio with multi_edit (comments trimmed there). TownBuilderTest now exists in Studio as a shortened copy (made with multi_edit); the disk file is the full one.
