...
9. Last RunAll in Studio: **418 passed, 0 failed** (10:56). WireDresser, CrossingDresser, CentreTreeDresser, GreeneryDresser, GatewayDresser, ClientKit, Planting, GroundInfluence (shared + client), Swans (client), LitWindows (client), PlayAreaLife (client) and their tests were created in Studio with multi_edit (comments trimmed there). TownBuilderTest now exists in Studio as a shortened copy (made with multi_edit); the disk file is the full one.
...
- Checked in play: 6 picnics built on the lawn, park summary 429 parts (budget 600). Close screenshot at (1118, 0, -161) checked. RunAll 418 passed.
