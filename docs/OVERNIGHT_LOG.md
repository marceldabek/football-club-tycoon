...
9. Last RunAll in Studio: **407 passed, 0 failed** (09:33). WireDresser, CrossingDresser, CentreTreeDresser, GroundInfluence (shared + client), Swans (client) and their tests were created in Studio with multi_edit (comments trimmed there). TownBuilderTest now exists in Studio as a shortened copy (made with multi_edit); the disk file is the full one.
