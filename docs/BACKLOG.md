...
- [ ] X98 Station terrace (X73) is 3 studs high, much lower than the tall wall in RivermereStation.png, and has no steps up to it. Consider raising it and adding a stair from the forecourt `[studio]` S
...
- [ ] X101 (08:40 not started: every shopfront is an uploaded texture, and a new one means uploading an image to Roblox, left for Marcel to OK) E2 list names a takeaway; the high street has none. A chippy or kebab shopfront with a lit sign among the ShopBuilder uses `[studio]` S
