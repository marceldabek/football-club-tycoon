...
- [ ] X86 (07:19 patches are now pairs of overlapping discs; the horizon edge is streaming: the first plot 2 visit showed hills and the river, the second a bare edge, so the picker camera is ahead of what has streamed. Still open: stream the backdrop for the picker, e.g. Persistent hill models or Player.ReplicationFocus on the viewed plot) From the plot 2 picker camera (PickerView_plot2) the ground ends in a flat edge on the horizon to the north-east, with bare lavender haze beyond and no backdrop hills or fields there; the lot's rough-grass patches also read as hard-edged rectangles. Extend the backdrop ring past the plot corners (check streaming: the picker camera may simply not have them streamed) and break the patches into irregular clusters `[studio]` S
